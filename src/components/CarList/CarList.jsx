...
import { FaArrowRight, FaGasPump, FaUsers, FaCog, FaLock } from 'react-icons/fa';
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {loading
            ...
            : displayedCars.map((car) => (
                ...
                >
                  {/* ⭐ Default badge for local cars */}
                  {car.isLocal && (
                    <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1 text-[10px] bg-black/80 dark:bg-white/10 backdrop-blur text-white px-3 py-1 rounded-full font-bold uppercase tracking-wider">
                      <FaLock size={8} /> Featured
                    </div>
                  )}
                  ...
                  <div className={`relative h-52 flex items-center justify-center overflow-hidden ${
                    ...
                    <img
                      ...
                      src={theme === 'dark' ? (car.imageDark || car.image) : car.image}
                      alt={car.name}
                      onError={(e) => {
                        e.target.onerror = null;
                        e.target.src = car.image;
                      }}
                    />
                  </div>
