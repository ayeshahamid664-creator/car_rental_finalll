...
const CarList = ({ theme, showAll = false }) => {
  ...
  // ⭐ Image resolver — dono casing handle
  const getImage = (car) => {
    if (theme === 'dark') {
      return car.image_dark || car.imageDark || car.image;
    }
    return car.image;
  };
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        {!loading && displayedCars.length === 0 && (
          <div className={`p-16 text-center rounded-3xl border ${
            ...
            <p className="font-serif text-xl font-bold mb-2">No cars available</p>
            <p className="text-sm opacity-60">Please add cars from the admin panel.</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {loading
            ...
            : displayedCars.map((car) => (
                <article
                  key={`${car.isOriginal ? 'orig' : 'admin'}-${car.id}`}
                ...
                >
                  ...
                  <div className={`relative h-52 flex items-center justify-center overflow-hidden ${
                    ...
                    {/* Mileage badge */}
                    <div className="absolute top-4 left-4 z-10 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/80 text-white text-[10px] font-semibold tracking-wide">
                      <FaGasPump size={9} />
                      {car.mileage || '12km'}
                    </div>
                    ...
                    <img
                      ...
                      loading="lazy"
                      onError={(e) => {
                        console.error('❌ Image failed:', car.name, getImage(car));
                        if (e.target.src !== car.image) e.target.src = car.image;
                      }}
                    />
                  </div>
        ...
        {!showAll && displayedCars.length > 0 && (
